...
The stdlib-only CLI is the `skills/docket/docket_cli/` package, run through the short launcher
`skills/docket/bin/docket`; role playbooks live in `skills/docket/references/`. `ARCHITECTURE.md` has the complete code and data map.
...
directories.
For a live Docket run, the planner syncs them after final approval so the tool under active sessions does not change.
The sync excludes generated bytecode:

```bash
rsync -a --exclude='__pycache__/' --exclude='*.pyc' ~/Documents/Projects/docket/skills/docket/ ~/.agents/skills/docket/
diff -r --exclude='__pycache__' --exclude='*.pyc' ~/.agents/skills/docket ~/Documents/Projects/docket/skills/docket
```

## House rules

- No dependencies. `bin/docket` and the `docket_cli` package are stdlib-only, and `bin/docket` must
  stay runnable as a plain `python3` script if someone swaps the shebang.
