...
For current behavior, [`skills/docket/docket_cli/`](../skills/docket/docket_cli/) and [`skills/docket/tests/test_docket.py`](../skills/docket/tests/test_docket.py) are the executable truth.
...
| [`docket_cli/`](../skills/docket/docket_cli/) | Stdlib-only implementation, one module per code-map region |
| [`bin/docket`](../skills/docket/bin/docket) | Launcher that runs the `docket_cli` package from cached bytecode |
...
- `events --peek` is inspection only; `watch` reconciles pending events and claims a bounded announcement lease before recording a wake.
...
- The configurable correction budget is enforced and exhaustion routes to the plan owner for an explicit grant.
