#!/usr/bin/env bash
# Stop-hook watcher arm for the handoff pipeline.
#
...
# a system reminder.
#
# It runs `handoff watch` in the FOREGROUND of this hook's own process tree, never
...
#
# Inert unless .handoff/watch.conf exists, so an idle project costs nothing.
# Arm:   echo "R01 orchestrator" > .handoff/watch.conf
# Disarm: rm .handoff/watch.conf
set -u

ROOT="${CLAUDE_PROJECT_DIR:-$PWD}"
CONF="$ROOT/.handoff/watch.conf"
...
cd "$ROOT" || exit 0
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")/../bin" && pwd)/handoff" \
  watch "$RUN" --role "${ROLE:-orchestrator}" --timeout "${HANDOFF_WATCH_TIMEOUT:-28800}"
